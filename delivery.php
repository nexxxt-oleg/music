<?php
include_once __DIR__ . '/inc/header.php'
?>
    <div class="container">
        <div class="row">
            <div class="col-12 music__page">
                <div class="music__page__breadcrumbs">
                    <ul class="nav">
                        <li><a href="">Главная</a></li>
                        <li><a href="">Оплата / Доставка</a></li>
                        <li><span>Доставка</span></li>
                    </ul>
                </div>
                <header class="d-flex justify-content-between music__page__header">
                    <h1 class="music__page__title">Доставка</h1>
                    <ul class="nav music__menu-head">
                        <li><a href="">Оплата</a></li>
                        <li><a href="" class="is_active">Доставка</a></li>
                    </ul>
                </header>
                <div class="music__text">
                    <p><b>Наш магазин осуществляет доставку по всей России!</b></p>
                </div>
                <div class="border-bottom fpad30">
                    <div class="row tpad15">
                        <div class="col-lg-3 fpad30">
                            <div class="music__page__card">
                                <div class="music__page__card__ico">
                                    <svg width="48" height="48" xmlns="http://www.w3.org/2000/svg"
                                         viewBox="0 0 512 511.98">
                                        <path fill="#d9c8b9"
                                              d="M150.24,376l-64-27.2a16,16,0,1,0-12.48,29.44l64,27.2A16,16,0,0,0,150.24,376Z"/>
                                        <path fill="#d9c8b9"
                                              d="M511.78,111.24a16.68,16.68,0,0,0-.8-4.89,14.91,14.91,0,0,0-1.51-2.4c-.35-.58-.61-1.09-1-1.6a15.28,15.28,0,0,0-6.66-5.15c-.09,0-.12-.13-.22-.16l-256-96a15.62,15.62,0,0,0-11.9.22l-224,96c-.23.1-.32.35-.55.45a15.5,15.5,0,0,0-5.6,4.7,16.21,16.21,0,0,0-1.21,1.73,13.09,13.09,0,0,0-1.06,1.54A16.09,16.09,0,0,0,0,112V400A16,16,0,0,0,9.7,414.7l224,96h0A15.94,15.94,0,0,0,240,512a16.24,16.24,0,0,0,5.63-1l.58-.23h0L501.63,415A16,16,0,0,0,512,400V112C512,111.69,511.78,111.5,511.78,111.24ZM224,471.69,32,389.42V136.27l192,82.27Zm16.38-281L56.64,112,240.38,33.26,450.43,112l-50.08,18.78L310.56,90.54a15.75,15.75,0,0,0-14.05.54l-62.75,26.66a16.05,16.05,0,0,0,.16,29.6l58.21,24ZM352,183.05l32-12v78l-32,12Zm-15.94-28.19L281.34,132.3,304,122.67l53.76,24.06ZM480,388.88l-224,84V219.05l64-24v89.09a16,16,0,0,0,16,16,16.37,16.37,0,0,0,5.63-1l64-24a16,16,0,0,0,10.37-15V159.05l64-24Z"/>
                                    </svg>
                                </div>
                                <div class="music__page__card__title">
                                    Самовывоз со<br>
                                    склада
                                </div>
                                <ul class="nav flex-column music__page__card__list">
                                    <li><b>Стоимость:</b> бесплатно</li>
                                    <li><b>Адрес:</b> г.Москва, п. Первомайское, д.Ботаково, складской комплекс
                                        "Ботаково"
                                    </li>
                                    <li><b>Часы работы:</b> с 10:00 до 18:00 кроме выходных и праздничных дней</li>
                                    <li><b>Оплата:</b> на сайте</li>
                                </ul>
                            </div>
                        </div>
                        <div class="col-lg-3 fpad30">
                            <div class="music__page__card">
                                <div class="music__page__card__ico">
                                    <svg width="48" height="48" xmlns="http://www.w3.org/2000/svg"
                                         viewBox="0 0 512 512">
                                        <path fill="#d9c8b9"
                                              d="M361.67,369V283.67H321.36V170.14L271,27.51V0H241V27.5L190.64,170.14V283.67H150.33V369H0V512H512V369ZM482,482H361.67V399h46v38.33h30V399H482Zm-243,0V405.33a17,17,0,1,1,34,0V482Zm17-123.67a47.05,47.05,0,0,0-47,47V482H180.33V313.67H331.67V482H303V405.33A47.05,47.05,0,0,0,256,358.33ZM30,399H74.33v38.33h30V399h46v83H30ZM256,213a35.36,35.36,0,1,1-35.36,35.36A35.4,35.4,0,0,1,256,213Zm35.36-19.57a65.15,65.15,0,0,0-70.72,0v-18.1L256,75.13l35.36,100.15Z"/>
                                    </svg>
                                </div>
                                <div class="music__page__card__title">
                                    Москва и Московская
                                    область
                                </div>
                                <ul class="nav flex-column music__page__card__list">
                                    <li><b>Стоимость:</b> бесплатно при заказе от 5000 руб.</li>
                                    <li><b>Способ доставки:</b> курьер, пункт выдачи</li>
                                    <li><b>Срок доставки:</b> 1 – 3 рабочих дня</li>
                                    <li><b>Оплата:</b> на сайте / при получении</li>
                                </ul>
                            </div>
                        </div>
                        <div class="col-lg-3 fpad30">
                            <div class="music__page__card">
                                <div class="music__page__card__ico">
                                    <svg width="52" height="48" xmlns="http://www.w3.org/2000/svg"
                                         viewBox="0 0 521 478.19">
                                        <path fill="#d9c8b9"
                                              d="M519.86,175.13C513,153.61,499.73,144,477,144l-9.35,0q-11.82,0-23.64,0c-3.18,0-6.43.16-9.56.33-7,.38-13.68.73-19.64-.83s-11.64-5.18-17.61-9c-2.6-1.66-5.28-3.37-8-4.92-10.92-6.18-21.84-12.39-32.86-18.66l-9.93-5.64,31.08-54.73a1.71,1.71,0,0,1,.23.13c4.52,2.5,12.92,7.15,19.14-2.57,2.18-3.42,2.93-6.68,2.23-9.7-1.11-4.79-5.43-7.4-8.9-9.5l-.26-.16.21-.36c1.67-2.94,3.25-5.71,4.79-8.56,2.08-3.85,2.63-7.7,1.59-11.14a11.79,11.79,0,0,0-6-7.07c-6.27-3.33-13.21-1-17.26,5.8-1.72,2.87-3.33,5.71-5,8.7l-.14.25c-4.38-2.47-13.48-7.59-19.67,3-6.29,10.78,2.67,16.06,7,18.59l.34.2-31.1,54.76C313.91,87,308.61,88.7,302.37,99.7l-2.2,3.88q-12.94,22.83-25.87,45.66c-5,8.87-3.72,14.53,4.63,20.17l9.39,6.36c10.49,7.1,21.34,14.44,32.1,21.55,24.34,16.1,42.61,37.13,54.31,62.53a13.32,13.32,0,0,1,1.2,5.28c.1,19.13.08,38.58.07,57.4v9.08c-8.87-.49-17.48,2.13-26.15,7.95-1.58,1.06-3.24,2-5,3-1.41.82-2.87,1.67-4.31,2.58-5.57,3.54-10.58,3.5-16.26-.13-3.61-2.3-7.59-4.69-12.18-7.31-13.22-7.53-26.54-7.45-39.58.26-1.83,1.08-3.66,2.18-5.48,3.28-2.54,1.54-5.18,3.13-7.78,4.62-4.6,2.66-8.89,2.64-13.52,0-2.74-1.59-5.52-3.26-8.21-4.88l-4.61-2.77c-13.62-8.1-27.41-8.06-41,.11-1.44.87-2.88,1.71-4.32,2.56-3,1.76-6.08,3.57-9.07,5.52-4.94,3.2-9.61,3.53-15.16,1.06-5.86-2.62-11.74-5-17.48-7a5.11,5.11,0,0,1-.89-.39,5.43,5.43,0,0,1-.08-1.08c.08-13,.09-26.25.1-39,0-11.46,0-23.31.08-35a13.41,13.41,0,0,1,1.24-5.28c10.4-22.67,25.22-41.14,44.06-54.91,11.49-8.39,23.56-16.47,35.23-24.29,5.58-3.74,11.35-7.61,17-11.47,7.63-5.21,9-11.09,4.54-19.05-10.8-19.17-20.56-36.36-29.83-52.58-4.72-8.25-11.19-10-19.78-5.25l-1.23.67L165.25,38.11l.05,0c4.32-2.52,13.31-7.76,7.49-18.4-3.24-5.92-7.53-7-10.56-6.84-3.33.16-6.36,1.83-9,3.3l-.31.17-3.94-6.89C141.84-3,133.52-.19,130.27,1.6a11.46,11.46,0,0,0-5.84,7c-1.06,3.76-.18,8,2.68,13.06,1.35,2.37,2.68,4.76,4,7.12-7,5.1-12.32,10.35-7.7,18.32.38.65,3.85,6.41,8.61,5.94A74.6,74.6,0,0,0,142.63,51l1-.25q13.05,23,26.11,46l4.82,8.49L164.49,111C146,121.51,126.8,132.42,107.9,143a10,10,0,0,1-4.46.85c-10,.06-20.19.05-30,.05-10.3,0-21,0-31.45.05-19.42.11-34.39,10.64-40.05,28.18-.3.94-.66,1.89-1,2.89l-.57,1.5-.28.76V352.37l.62,1.05c4.49,7.66,12.22,7.63,14.78,7.63,37-.13,74-.13,109.94,0a27.85,27.85,0,0,1,9.11,1.83c4.67,1.65,9.38,3.58,13.94,5.45,2.26.92,4.52,1.85,6.78,2.75a41.2,41.2,0,0,0,15.25,3.07,38.43,38.43,0,0,0,20.33-6q3.28-2,6.59-3.94c2.48-1.47,5-3,7.56-4.54,5.17-3.19,9.95-3.22,15-.08,3,1.84,6,3.66,9,5.42l3,1.81c13.58,8.12,27.37,8.13,41,0,2.08-1.24,4.16-2.5,6.23-3.75s4.12-2.48,6.19-3.71c4.87-2.9,9.41-2.89,14.29,0q2.24,1.35,4.46,2.71c2.89,1.75,5.88,3.57,8.89,5.3,12.6,7.22,25.44,7.35,38.16.41,5.34-2.92,10.5-6.15,14.4-8.63,5-3.17,9.6-3.41,15-.78a24.73,24.73,0,0,0,10.52,2.45c19.26.07,38.83.08,57.75.1,18.6,0,37.83,0,56.73.09h.1c6.86,0,11.66-2.51,14.68-7.67l.61-1.05v-176ZM374.52,186.64c1.53.85,1.59,1.08,1.56,2.61-.13,8.23-.12,16.48-.1,25.14-10.34-12.93-20.85-23-32.47-31.26-10.57-7.5-21.53-14.9-32.12-22.06q-5.43-3.67-10.9-7.39l3.89-6.79.91.51,10.14,5.75,14.92,8.49C344.8,169.88,359.75,178.39,374.52,186.64Zm-57.78-61.52,3.44-6.06L386.92,157l-4.43,5.52ZM496,335.61v.11H401V294.56q0-54.35,0-108.71c0-10.64,6.07-16.78,16.63-16.83,22.1-.12,42.2-.13,61.44,0C490,169,496,175.13,496,186.14Q496,260.87,496,335.61Zm-376,.12H25V187.16C25,174.59,30.66,169,43.34,169c21,0,40.13,0,58.47,0,12.58,0,18.19,5.68,18.18,18.35q0,53.63,0,107.25Zm18.51-173.26L134.08,157l66.74-37.88,3.45,6Zm6.61,26.17a3.2,3.2,0,0,1,.39-1.15c20.52-11.84,40.67-23.3,62-35.42l9.13-5.19,3.9,6.89c-5.51,3.81-11.16,7.55-16.66,11.19C183.09,178.7,161.7,192.84,145,213v-.4C145,204.76,145,196.62,145.12,188.64Z"/>
                                        <path fill="#d9c8b9"
                                              d="M500,398.62c-5.34,2.7-10.21,2.62-15.33-.26-1.89-1.06-3.81-2.23-5.67-3.35s-3.62-2.19-5.48-3.25c-14.09-8-28.35-7.91-42.4.3l-1.48.87c-2.52,1.46-5.12,3-7.65,4.57-6.41,4-12.36,4.06-18.71.1-3.63-2.27-7.31-4.44-11-6.46-9.87-5.48-18.72-6.86-27-4.23a114.12,114.12,0,0,0-22.46,10.27c-7.5,4.33-13.84,4.28-20.57-.15-2.9-1.92-6-3.8-9.29-5.59-14-7.65-27.89-7.52-41.47.4l-1.12.65c-2.64,1.54-5.38,3.13-8,4.84-6.73,4.32-13.06,4.25-19.92-.21-3.34-2.17-6.78-4.2-10.2-6-13.28-7.12-26.75-7-40,.3-3.16,1.74-6.42,3.7-9.69,5.82-6.89,4.47-13.34,4.41-20.29-.21-2.91-1.93-6-3.68-8.84-5.3-13.92-7.86-28-7.81-41.93.16-1.66.95-3.3,1.93-4.94,2.91-2,1.22-4.13,2.48-6.2,3.63a15,15,0,0,1-15.62,0c-2-1.1-4-2.31-5.93-3.48-1.69-1-3.44-2.08-5.22-3.08-14.15-8.06-28.41-8-42.4.28l-2.54,1.49C22,397.4,15.24,401.38,8.7,405.51A32.55,32.55,0,0,0,3.37,410c-.62.61-1.24,1.22-1.88,1.79L0,413.14v8.47l.86,1.18c3.62,5,7.07,7.4,11.16,7.4,2.77,0,5.83-1.1,9.44-3.25q6.76-4,13.49-8.08l8.79-5.27c6-3.58,11.56-3.57,17.57,0l3.07,1.85c2.66,1.61,5.42,3.27,8.21,4.83,9.89,5.53,19.33,6.92,28.06,4.13a106,106,0,0,0,21.52-10.06c7.46-4.39,13.79-4.37,20.53.05,3.13,2,6.47,4,10.21,6,12.87,6.93,26,7,39.08.07l.59-.31a36.44,36.44,0,0,0,5.65-3.43c9.69-7.54,18.82-7.51,28.74.12a34.87,34.87,0,0,0,5.08,3.09l.69.36c13.21,7.08,26.52,7.08,39.55,0,3-1.61,5.85-3.37,8.63-5.07l2.71-1.65c6-3.61,11.58-3.64,17.58-.09L305,415.8c2.44,1.47,5,3,7.54,4.42,10.12,5.64,19.73,7,28.56,4.13,7.68-2.52,15-6.48,21.83-10.33s12.8-3.95,18.93-.11c1.23.78,2.47,1.5,3.68,2.2s2.43,1.42,3.59,2.16c15.45,9.78,31.13,9.84,46.58.16,1.73-1.09,3.56-2.15,5.32-3.19.92-.53,1.83-1.07,2.74-1.61,6-3.56,11.58-3.56,17.62,0q1.92,1.14,3.82,2.3c2.7,1.65,5.5,3.36,8.41,4.9,15.59,8.25,35.49,5.37,46.28-6.69l1.15-1.28v-9.42l-1.13-1.28C513.27,394.69,509.17,394,500,398.62Z"/>
                                        <path fill="#d9c8b9"
                                              d="M500,446.59a15.33,15.33,0,0,1-15.35-.23c-1.93-1.08-3.9-2.26-5.79-3.41s-3.55-2.13-5.37-3.16c-14.11-8-28.38-7.93-42.4.29-1.64,1-3.27,1.94-4.9,2.92s-3.29,2-4.95,2.95c-5.89,3.45-11.38,3.47-17.26.06-1.5-.87-3-1.77-4.47-2.66-2.34-1.41-4.76-2.86-7.23-4.22-9.93-5.47-18.78-6.85-27.07-4.19s-16.42,6.93-23.25,10.69-12.52,3.75-18.55,0c-3.49-2.15-6.92-4.26-10.51-6.22C299,431.83,285,432,271.4,439.87l-.38.22c-3,1.75-6.16,3.56-9.2,5.48-6.44,4.05-12.51,4-19.08-.26-2.71-1.74-5.64-3.49-9.21-5.52-14.17-8-28.44-7.94-42.4.24-1.4.81-2.78,1.64-4.17,2.47-2.13,1.28-4.34,2.6-6.51,3.83a15.19,15.19,0,0,1-16-.06c-1.88-1.07-3.79-2.22-5.64-3.34-1.68-1-3.36-2-5.06-3-14.07-8.12-28.34-8.12-42.4,0-2.64,1.52-5.28,3.11-7.82,4.65l-2.11,1.27c-6.14,3.68-11.84,3.65-18-.1l-1.08-.67c-3-1.86-6.17-3.79-9.37-5.57-14-7.78-28.06-7.58-41.93.58l-1.8,1.06c-6.78,4-13.78,8.11-20.56,12.4A32.1,32.1,0,0,0,3.37,458c-.62.61-1.23,1.21-1.87,1.79L0,461.15v8.47l.86,1.18c3.62,5,7.06,7.39,11.16,7.39,2.77,0,5.83-1.09,9.44-3.23q4.66-2.77,9.31-5.58c4.37-2.63,8.9-5.36,13.38-8,5.62-3.29,11.12-3.29,16.8,0,1.51.87,3,1.78,4.48,2.69,2.48,1.51,5,3.08,7.71,4.49,12.78,6.76,25.77,6.78,38.61.07,3-1.59,6-3.38,8.78-5.11l3.06-1.86c6-3.6,11.6-3.64,17.63-.12,1.39.81,2.76,1.65,4.13,2.48,2.31,1.4,4.7,2.84,7.15,4.18,13.28,7.26,26.75,7.25,40,0,3.62-2,7.14-4.14,10.56-6.23l.37-.23c6.1-3.73,11.79-3.74,17.94,0l.49.3c3.08,1.86,6.27,3.79,9.47,5.62,14.09,8.05,28.36,8,42.41-.1,3.31-1.9,6.52-3.82,9.55-5.69,6.25-3.87,12.07-3.88,18.29,0l.4.25c3.56,2.2,7.24,4.49,11.07,6.49,13.2,6.92,26.51,6.77,39.56-.42,3.93-2.16,7.68-4.47,10.51-6.23,6.37-4,12.32-4,18.73,0l.2.13c3.47,2.16,7.06,4.4,10.79,6.39,12.94,6.91,26.09,6.93,39.08.07,3.19-1.69,6.47-3.63,10.62-6.3,6.78-4.35,13.12-4.34,19.94.06,4.32,2.79,8,4.94,11.53,6.8,15.38,8,35.07,5,45.8-6.94l1.15-1.29v-9.42l-1.13-1.28C513.26,442.69,509.16,442,500,446.59Z"/>
                                    </svg>
                                </div>
                                <div class="music__page__card__title">
                                    Санкт-Петербург и
                                    Ленинградская область
                                </div>
                                <ul class="nav flex-column music__page__card__list">
                                    <li><b>Стоимость:</b> от 200 руб.</li>
                                    <li><b>Способ доставки:</b> курьер, пункт выдачи</li>
                                    <li><b>Срок доставки:</b> 2 – 3 рабочих дня</li>
                                    <li><b>Оплата:</b> на сайте</li>
                                </ul>
                            </div>
                        </div>
                        <div class="col-lg-3 fpad30">
                            <div class="music__page__card">
                                <div class="music__page__card__ico">
                                    <svg width="56" height="48" xmlns="http://www.w3.org/2000/svg"
                                         viewBox="0 0 512 432.62">
                                        <path fill="#d9c8b9"
                                              d="M139.64,155.2a50.07,50.07,0,1,0-50.06-50.06A50.11,50.11,0,0,0,139.64,155.2Zm0-70a20,20,0,1,1-20,20A20,20,0,0,1,139.64,85.15Z"/>
                                        <path fill="#d9c8b9"
                                              d="M461.24,301.13H440.83a28.58,28.58,0,0,1,4.24-10.24c7.65-11.85,9.28-22.31,10.72-31.54.91-5.82,1.77-11.32,4.11-17.66C462,236.08,466,231.52,470,228l14.11,34.77L512,251.5,461.57,126.8l-12.25,2.48c-1.4.28-34.5,7.37-55.19,45.69-15.1,28-49.11,49.68-90.48,58.39l-8.24-30.26-14.71,4.31c-28.44,8.32-42.92,15.67-72,33.53-9.94,6.1-23.21,9.84-39.65,11.23,7.32-9.82,27.28-36.2,27.28-36.2,20.45-26.76,48.47-63.41,48.47-110.83a105.14,105.14,0,1,0-210.27,0c0,47.42,28,84.07,48.47,110.83,0,0,18.6,24.56,25.57,33.91-23.53-.39-41.86,5.94-55.76,19.21C36.58,284.57,30,310.18,35.19,333.61L0,357l41.61,75.25H61.79l15.59-17.55a83,83,0,0,1,116.53-7.47c17.25,15,34,23.36,49.79,24.94a85.46,85.46,0,0,0,8.54.44c14.65,0,25.42-4,35.11-7.54,10.92-4,19.55-7.21,34.07-5.32a132.09,132.09,0,0,0,16.83,1.17c28.7,0,42.46-12,53.3-29.75a25.25,25.25,0,0,1,13.95-11.24c11-3.45,25,11.18,25,11.18l28.41-14a126.34,126.34,0,0,1,7.63,43.33h30.07a156.52,156.52,0,0,0-11.23-58.34Zm-321.6-271a75.14,75.14,0,0,1,75.06,75.06c0,37.24-23.45,67.92-42.29,92.57,0,0-24.69,32.69-32.77,43.59-8.08-10.9-32.77-43.59-32.77-43.59C88,173.06,64.58,142.38,64.58,105.14a75.14,75.14,0,0,1,75.06-75.06ZM433.07,356.36c-13.59-7.47-25.37-7.5-33.35-5.94-13.58,2.66-25.61,11.58-33.85,25.11-6,9.78-11.11,18.22-40.59,14.4-21.86-2.83-35.92,2.35-48.33,6.92-10,3.71-18,6.63-30.25,5.4-9.35-.93-20.79-7.06-33.07-17.72a113.05,113.05,0,0,0-158.53,10L39.73,366.71l31.91-21.22L66.76,334c-7.2-16.94-1.87-34.88,6.8-43.16,9.94-9.49,24-12.61,45.51-10.13,46.61,5.38,81.07.75,105.34-14.14,21.61-13.26,33.53-19.81,50.13-25.65l7.17,26.31,13.1-1.82c57.32-8,104.34-36.45,125.78-76.15,7.27-13.46,16.38-21.18,23.47-25.49l14.27,35.44c-9.31,6.21-20.95,16.61-26.66,32.08a114,114,0,0,0-5.59,23.41c-1.26,8.07-2.1,13.39-6.28,19.88-16.17,25.06-6.89,45.86-5.78,48.15l4.11,8.47h22.62l7.06,17.91Z"/>
                                    </svg>
                                </div>
                                <div class="music__page__card__title">
                                    Другие регионы<br>
                                    России
                                </div>
                                <ul class="nav flex-column music__page__card__list">
                                    <li><b>Стоимость:</b> от 200 руб., рассчитывается автоматически при оформлении
                                        заказа
                                    </li>
                                    <li><b>Способ доставки:</b> курьер, пункт выдачи</li>
                                    <li><b>Срок доставки:</b> от 2 рабочих дней</li>
                                    <li><b>Оплата:</b> на сайте</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="row tpad50 fpad30">
                    <div class="col-12 music__q-title music__text tpad30">
                        <h2 class="text-uppercase">Часто задаваемые вопросы</h2>
                    </div>
                    <div class="col-xl-3 col-lg-4 col-sm-5 fpad30 music__col-q">
                        <ul class="nav flex-column music__sb-left">
                            <li class="nav-item">
                                <a class="nav-link active" data-bs-toggle="tab" href="#tab1">Самовывоз со склада </a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" data-bs-toggle="tab" href="#tab2">Москва и Московская область</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" data-bs-toggle="tab" href="#tab3">Санкт-Петербург и Ленинградская
                                    область</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" data-bs-toggle="tab" href="#tab4">Другие регионы России</a>
                            </li>
                        </ul>
                    </div>
                    <div class="col-xl-9 col-lg-8 col-sm-7 music__main__text fpad30 music__col-tab-q">
                        <div class="tab-content">
                            <div class="tab-pane fade show active accordion" id="tab1">
                                <div class="music__col-tab-q__caption">Самовывоз со склада</div>
                                <div class="music__box-rev">
                                    <div class="music__box-rev__title" data-bs-toggle="collapse"
                                         data-bs-target="#collapse1_tab1">
                                        Есть ли система скидок постоянным покупателям?
                                    </div>
                                    <div id="collapse1_tab1" class="collapse show" >
                                        <div class="music__box-rev__body">
                                            <p>Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus
                                                terry
                                                richardson ad squid. 3 wolf moon officia aute, non cupidatat skateboard
                                                dolor
                                                brunch. Food truck quinoa nesciunt laborum eiusmod. Brunch 3 wolf moon
                                                tempor,
                                                sunt aliqua put a bird on it squid single-origin coffee nulla assumenda
                                                shoreditch et. Nihil anim keffiyeh helvetica, craft beer labore wes
                                                anderson
                                                cred nesciunt sapiente ea proident. Ad vegan excepteur butcher vice
                                                lomo.
                                                Leggings occaecat craft beer farm-to-table, raw denim aesthetic synth
                                                nesciunt
                                                you probably haven't heard of them accusamus labore sustainable VHS.</p>
                                        </div>
                                    </div>
                                </div>
                                <div class="music__box-rev">
                                    <div class="music__box-rev__title collapsed" data-bs-toggle="collapse"
                                         data-bs-target="#collapse2_tab1">
                                        Бывают ли у вас Акции? как найти товар по Акции?
                                    </div>
                                    <div id="collapse2_tab1" class="collapse" ">
                                        <div class="music__box-rev__body">
                                            <p>При входе на сайт ваш город определится автоматически, Акции по
                                                федеральным округам России могут различаться.</p>
                                            <p>В интернет-магазине КИБЕРСЛОН регулярно проводятся Акции на товары и
                                                услуги. Узнать подробно об Акциях и сроках их проведения можно из
                                                Главного меню. При клике на кнопку "% Акции" перейдите на список
                                                Акций.</p>
                                            <p>Товары по Акции имеют 2 цены, прежняя цена перечеркнута, новая цена
                                                отражена, на желтом стикере отмечена сумма вашей экономии при
                                                покупке.</p>
                                            <p>Товары по Акции могут быть обозначены специальным стикером "Акция". В
                                                разделе необходимого вам товара можно воспользоваться "умным" фильтром и
                                                отсортировать все товары с таким стикером.</p>
                                            <p>Отдельные Акции проходят с применением Купона (промо-кода) на скидку. В
                                                Корзине товаров внесите название полученного купона. Скидка на товар
                                                пересчитается автоматически.</p>
                                            <p>Приятных покупок!</p>
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <div class="tab-pane fade" id="tab2">
                                <div class="music__col-tab-q__caption">Москва и Московская область</div>
                                <div class="music__box-rev">
                                    <div class="music__box-rev__title" data-toggle="collapse"
                                         data-target="#collapse1_tab2">
                                        Есть ли система скидок постоянным покупателям?
                                    </div>
                                    <div id="collapse1_tab2" class="collapse show" data-parent="#tab2">
                                        <div class="music__box-rev__body">
                                            <p>Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus
                                                terry
                                                richardson ad squid. 3 wolf moon officia aute, non cupidatat skateboard
                                                dolor
                                                brunch. Food truck quinoa nesciunt laborum eiusmod. Brunch 3 wolf moon
                                                tempor,
                                                sunt aliqua put a bird on it squid single-origin coffee nulla assumenda
                                                shoreditch et. Nihil anim keffiyeh helvetica, craft beer labore wes
                                                anderson
                                                cred nesciunt sapiente ea proident. Ad vegan excepteur butcher vice
                                                lomo.
                                                Leggings occaecat craft beer farm-to-table, raw denim aesthetic synth
                                                nesciunt
                                                you probably haven't heard of them accusamus labore sustainable VHS.</p>
                                        </div>
                                    </div>
                                </div>
                                <div class="music__box-rev">
                                    <div class="music__box-rev__title collapsed" data-toggle="collapse"
                                         data-target="#collapse2_tab2">
                                        Бывают ли у вас Акции? как найти товар по Акции?
                                    </div>
                                    <div id="collapse2_tab2" class="collapse" data-parent="#tab2">
                                        <div class="music__box-rev__body">
                                            <p>При входе на сайт ваш город определится автоматически, Акции по
                                                федеральным округам России могут различаться.</p>
                                            <p>В интернет-магазине КИБЕРСЛОН регулярно проводятся Акции на товары и
                                                услуги. Узнать подробно об Акциях и сроках их проведения можно из
                                                Главного меню. При клике на кнопку "% Акции" перейдите на список
                                                Акций.</p>
                                            <p>Товары по Акции имеют 2 цены, прежняя цена перечеркнута, новая цена
                                                отражена, на желтом стикере отмечена сумма вашей экономии при
                                                покупке.</p>
                                            <p>Товары по Акции могут быть обозначены специальным стикером "Акция". В
                                                разделе необходимого вам товара можно воспользоваться "умным" фильтром и
                                                отсортировать все товары с таким стикером.</p>
                                            <p>Отдельные Акции проходят с применением Купона (промо-кода) на скидку. В
                                                Корзине товаров внесите название полученного купона. Скидка на товар
                                                пересчитается автоматически.</p>
                                            <p>Приятных покупок!</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="tab-pane fade" id="tab3">
                                <div class="music__col-tab-q__caption">Санкт-Петербург и Ленинградская
                                    область
                                </div>
                                <div class="music__box-rev">
                                    <div class="music__box-rev__title" data-toggle="collapse"
                                         data-target="#collapse1_tab3">
                                        Есть ли система скидок постоянным покупателям?
                                    </div>
                                    <div id="collapse1_tab3" class="collapse show" data-parent="#tab3">
                                        <div class="music__box-rev__body">
                                            <p>Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus
                                                terry
                                                richardson ad squid. 3 wolf moon officia aute, non cupidatat skateboard
                                                dolor
                                                brunch. Food truck quinoa nesciunt laborum eiusmod. Brunch 3 wolf moon
                                                tempor,
                                                sunt aliqua put a bird on it squid single-origin coffee nulla assumenda
                                                shoreditch et. Nihil anim keffiyeh helvetica, craft beer labore wes
                                                anderson
                                                cred nesciunt sapiente ea proident. Ad vegan excepteur butcher vice
                                                lomo.
                                                Leggings occaecat craft beer farm-to-table, raw denim aesthetic synth
                                                nesciunt
                                                you probably haven't heard of them accusamus labore sustainable VHS.</p>
                                        </div>
                                    </div>
                                </div>
                                <div class="music__box-rev">
                                    <div class="music__box-rev__title collapsed" data-toggle="collapse"
                                         data-target="#collapse2_tab1">
                                        Бывают ли у вас Акции? как найти товар по Акции?
                                    </div>
                                    <div id="collapse2_tab3" class="collapse" data-parent="#tab3">
                                        <div class="music__box-rev__body">
                                            <p>При входе на сайт ваш город определится автоматически, Акции по
                                                федеральным округам России могут различаться.</p>
                                            <p>В интернет-магазине КИБЕРСЛОН регулярно проводятся Акции на товары и
                                                услуги. Узнать подробно об Акциях и сроках их проведения можно из
                                                Главного меню. При клике на кнопку "% Акции" перейдите на список
                                                Акций.</p>
                                            <p>Товары по Акции имеют 2 цены, прежняя цена перечеркнута, новая цена
                                                отражена, на желтом стикере отмечена сумма вашей экономии при
                                                покупке.</p>
                                            <p>Товары по Акции могут быть обозначены специальным стикером "Акция". В
                                                разделе необходимого вам товара можно воспользоваться "умным" фильтром и
                                                отсортировать все товары с таким стикером.</p>
                                            <p>Отдельные Акции проходят с применением Купона (промо-кода) на скидку. В
                                                Корзине товаров внесите название полученного купона. Скидка на товар
                                                пересчитается автоматически.</p>
                                            <p>Приятных покупок!</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="tab-pane fade" id="tab4">
                                <div class="music__col-tab-q__caption">Другие регионы России</div>
                                <div class="music__box-rev">
                                    <div class="music__box-rev__title" data-toggle="collapse"
                                         data-target="#collapse1_tab4">
                                        Есть ли система скидок постоянным покупателям?
                                    </div>
                                    <div id="collapse1_tab4" class="collapse show" data-parent="#tab4">
                                        <div class="music__box-rev__body">
                                            <p>Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus
                                                terry
                                                richardson ad squid. 3 wolf moon officia aute, non cupidatat skateboard
                                                dolor
                                                brunch. Food truck quinoa nesciunt laborum eiusmod. Brunch 3 wolf moon
                                                tempor,
                                                sunt aliqua put a bird on it squid single-origin coffee nulla assumenda
                                                shoreditch et. Nihil anim keffiyeh helvetica, craft beer labore wes
                                                anderson
                                                cred nesciunt sapiente ea proident. Ad vegan excepteur butcher vice
                                                lomo.
                                                Leggings occaecat craft beer farm-to-table, raw denim aesthetic synth
                                                nesciunt
                                                you probably haven't heard of them accusamus labore sustainable VHS.</p>
                                        </div>
                                    </div>
                                </div>
                                <div class="music__box-rev">
                                    <div class="music__box-rev__title collapsed" data-toggle="collapse"
                                         data-target="#collapse2_tab4">
                                        Бывают ли у вас Акции? как найти товар по Акции?
                                    </div>
                                    <div id="collapse2_tab4" class="collapse" data-parent="#tab4">
                                        <div class="music__box-rev__body">
                                            <p>При входе на сайт ваш город определится автоматически, Акции по
                                                федеральным округам России могут различаться.</p>
                                            <p>В интернет-магазине КИБЕРСЛОН регулярно проводятся Акции на товары и
                                                услуги. Узнать подробно об Акциях и сроках их проведения можно из
                                                Главного меню. При клике на кнопку "% Акции" перейдите на список
                                                Акций.</p>
                                            <p>Товары по Акции имеют 2 цены, прежняя цена перечеркнута, новая цена
                                                отражена, на желтом стикере отмечена сумма вашей экономии при
                                                покупке.</p>
                                            <p>Товары по Акции могут быть обозначены специальным стикером "Акция". В
                                                разделе необходимого вам товара можно воспользоваться "умным" фильтром и
                                                отсортировать все товары с таким стикером.</p>
                                            <p>Отдельные Акции проходят с применением Купона (промо-кода) на скидку. В
                                                Корзине товаров внесите название полученного купона. Скидка на товар
                                                пересчитается автоматически.</p>
                                            <p>Приятных покупок!</p>
                                        </div>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                </div>

            </div>
        </div>
    </div>
    <div class="fpad70 tpad70">
        <?php
        include __DIR__ . '/inc/callback.php';
        ?>
    </div>
<?php
include_once __DIR__ . '/inc/footer.php';
?>